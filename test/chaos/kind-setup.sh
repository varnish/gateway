...
# Traffic is now generated by an in-cluster k6 Job (see test/chaos/manifests/
# k6-job.yaml); pre-pull the image and load it into kind so scenarios don't
# try to pull it from docker.io on every run.
echo "==> loading k6 image ($k6_image) into kind"
if ! docker image inspect "$k6_image" >/dev/null 2>&1; then
  docker pull "$k6_image"
fi
go tool kind load docker-image "$k6_image" --name "$cluster"
...
echo
echo "Cluster ready. Traffic is generated in-cluster — no port-forwards needed."
echo
echo "Run a single scenario:"
echo "  make chaos-run SCENARIO=C01"
echo
echo "Run the whole suite (safe to leave overnight):"
echo "  make chaos-suite"
echo
